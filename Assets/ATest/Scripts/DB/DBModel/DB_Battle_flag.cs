...
public class DB_Battle_flag : IDB_BaseString
{
	const string SQL = @"select _index, key, map, load_pos from conf_battle_flag";
	...
	static Dictionary<string, IDB_BaseString> LoadDB()
	{
		...
		DBHelper.Instance.Query(SQL, (reader) =>
		{
			...
			while (reader.Read())
			{
				DB_Battle_flag module = new DB_Battle_flag()
				{
