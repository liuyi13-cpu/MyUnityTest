    ...
    public class AssetBundleManager : MonoBehaviour
    {
        ...
        private static void Log(LogType logType, string text)
        {
            if (logType == LogType.Error)
                Debugger.LogError("[AssetBundleManager] " + text);
            else if (m_LogMode == LogMode.All && logType == LogType.Warning)
                Debugger.LogWarning("[AssetBundleManager] " + text);
            else if (m_LogMode == LogMode.All)
                Debugger.Log("[AssetBundleManager] " + text);
        }
        ...
        /// 同步加载AB
        /// </summary>
        static public T LoadAsset<T>(string assetBundleName, string assetName) where T : UnityEngine.Object
        {
            Log(LogType.Info, "Loading " + assetName + " from " + assetBundleName + " bundle");
            ...
            if (SimulateAssetBundleInEditor)
            {
                ...
                // @TODO: Now we only get the main object from the first asset. Should consider type also.
                T target = AssetDatabase.LoadMainAssetAtPath(assetPaths[0]) as T;
                return target;
            }
            else
#endif
            {
                ...
                {
                    Log(LogType.Error, "Load Asset Error:" + error);
                    return null;
                }

                if (bundle == null || bundle.m_AssetBundle == null)
                {
                    Log(LogType.Error, "同步加载时无法找到AB [" + assetBundleName + "] 移动端不支持直接载入Bundle，请先加载对应Bundle。在PC上请打开 Simulate Model");
                    return null;
                }

                T target = bundle.m_AssetBundle.LoadAsset<T>(assetName);
                return target;
            }
        }
